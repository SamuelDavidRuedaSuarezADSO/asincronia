...
async function ver() {
  ...
  todosRepos.forEach(entry => {  // Recorre cada entrada en el arreglo de todos los repositorios
    if (entry.repositorios.length < 5) {  // Verifica si el usuario tiene menos de 5 repositorios
      ...
      console.table(entry.repositorios.map(repo => ({ Nombre: repo.name, Descripción: repo.description })));  // Muestra los repositorios en una tabla en la consola
    }
  });
  ...
  todosRepos.forEach(entry => {  // Recorre cada entrada en el arreglo de todos los repositorios
    ...
    if (reposConJS.length > 0) {  // Verifica si hay repositorios que contienen "JavaScript"
      ...
      console.table(reposConJS.map(repo => ({ Nombre: repo.name, Descripción: repo.description })));  // Muestra los repositorios en una tabla en la consola
    }
  });
  ...
  todosRepos.forEach(entry => {  // Recorre cada entrada en el arreglo de todos los repositorios
    ...
    console.table(reposOrdenados.map(repo => ({ Nombre: repo.name, Descripción: repo.description })));  // Muestra los repositorios ordenados en una tabla en la consola
  });
  ...
  todosRepos.forEach(entry => {  // Recorre cada entrada en el arreglo de todos los repositorios
    ...
    if (reposLargos.length > 0) {  // Verifica si hay repositorios con nombres de más de cinco letras
      ...
      console.table(reposLargos.map(repo => ({ Nombre: repo.name, Descripción: repo.description })));  // Muestra los repositorios en una tabla en la consola
    }
  });
}
